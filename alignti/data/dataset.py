...
from alignti.utils import order_pick_k
from alignti.data.data_utils import *
...
def download_image_from_oss(loc_path, crop_size):
    oss_prefix = 'http://kjyj-yunsheng.oss-cn-hangzhou-zmf.aliyuncs.com/changyou/'
    loc_path = loc_path[loc_path.find('datasets'):]
    path = f"{oss_prefix}{loc_path}"
    try:
        response = requests.get(path)
        if response.status_code == 200:
            image = Image.open(BytesIO(response.content)).convert('RGB')
        else:
            image = Image.new('RGB', (crop_size['height'], crop_size['width']), (0, 0, 0))
    except Exception as e:
        print(f'download image from oss error: {e}')
        image = Image.new('RGB', (crop_size['height'], crop_size['width']), (0, 0, 0))
    return image
        ...
        try:
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
                ...
                for file in image_file:
                    try:
                        loc_path = os.path.join(image_folder, file)
                        img = download_image_from_oss(loc_path, crop_size=image_processor.size)
                        # img = Image.open(os.path.join(loc_path)).convert('RGB')
                        image.append(img)
                    except Exception as e:
                ...
                image_file = order_pick_k(image_file, MAX_IMAGE_LENGTH)
                image = [
                    download_image_from_oss(os.path.join(image_folder, file), image_processor.size) for file in image_file
                ]
                if self.data_args.image_aspect_ratio == 'pad':
                    image = [expand2square(i, tuple(int(x * 255) for x in image_processor.image_mean)) for i in image]
                    image = [image_processor.preprocess(i, return_tensors='pt')['pixel_values'][0] for i in image]
