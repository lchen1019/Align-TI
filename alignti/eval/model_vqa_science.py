...
from alignti.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from alignti.conversation import conv_templates, SeparatorStyle
from alignti.model.builder import load_pretrained_model
from alignti.utils import disable_torch_init
from alignti.mm_utils import tokenizer_image_token, get_model_name_from_path, KeywordsStoppingCriteria
...
def eval_model(args):
    ...
    if args.return_gating_logit is not None:
        from alignti.utils import get_gating_logit_by_hook
        print(model)
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        if outputs.endswith('<|endoftext|>'):
            outputs = outputs.removesuffix('<|endoftext|>')
        outputs = outputs.strip()
