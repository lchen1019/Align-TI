...
from alignti.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, \
            ...
            def get_w(weights, keyword):
                ...
                for k, v in weights.items():
                    if keyword in k:
                        ...
                        if 'image_spatial_proj' not in k:
                            new_k = 'image_spatial_proj.' + new_k
                        rets[new_k] = v
                return rets
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_labels.append(
                        torch.full((cur_image_features.shape[0],), IMAGE_TOKEN_INDEX, device=cur_labels.device,
        ...
        # Combine them，维度不一样，取出最大的，其余填充为IGNORE_INDEX，从而可以构成batch
        ...
        if new_labels is not None:
            new_vision_token_tag = new_labels.clone()
            new_labels[new_labels==IMAGE_TOKEN_INDEX] = IGNORE_INDEX
        else:
            new_vision_token_tag = None

        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, new_vision_token_tag
